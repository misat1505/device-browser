...
export function DevicePrice({ price, validFrom, validTo }: DevicePriceProps) {
  return (
    ...
    >
      <p className="m-0 text-[12px] leading-4.5 font-normal text-[#767676] -translate-y-0.5">
        Cena obowiązuje: {formatDate(validFrom)} - {formatDate(validTo)}
      </p>

      <div className="flex h-10 items-center gap-0.5 -mt-px">
        <span className="text-[40px] leading-10 font-bold text-black -translate-y-0.5">
          {formatPrice(price)}
        </span>

        <span className="w-4.75 text-right text-[14px] leading-4 font-bold text-black -translate-y-0.5">
